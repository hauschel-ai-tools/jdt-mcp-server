...
| JUnit-Tests laufen weder mit JUnit Platform 1.x noch 6.x | `jdt_run_tests`, `jdt_start_tests_async` | Platform 1.x: `NoClassDefFoundError: OutputDirectoryCreator` im JUnit5-Runner. Platform 6.x: JDT-Precheck lehnt den fest eingestellten junit5-Loader ab ("Cannot find Testable on project build path"). Workaround: `jdt_maven_build(goals="test")` | [#71](https://git.changinggraph.org/ai-tools/jdt-mcp-server/issues/71), [#85](https://git.changinggraph.org/ai-tools/jdt-mcp-server/issues/85) |
| Testklassen werden nach `target/classes` kompiliert | `jdt_import_project` | Test-Source-Folder bekommen keinen eigenen Output-Ordner. Ein `mvn package` ohne `clean` packt Testklassen ins Jar und erzeugt "Unresolved compilation problems" in anderen Modulen. Workaround: bei verbundenem Server immer `mvn clean …` | [#84](https://git.changinggraph.org/ai-tools/jdt-mcp-server/issues/84) |
